...
\end{align}
You might want to compare this to BH 7.2.2, where they analyze the case with uniform $X, Y$.

However, as we will see in the first set of exercises below, this is \emph{ not true for discrete} memoryless r.v.s, i.e., when $X, Y \sim \Geo{p}$, while it is true when $X, Y \sim \Exp{\lambda}$, cf., BH Ex.5.6.5.\footnote{BH: Blitzstein and Hwang}
...
So we redo the work of~\cref{sec:exerc-expon-distr}, but now with change of variables techniques and densities. Read it once you study BH ch 8.
...
\subsection{Working with densities}
...
\begin{align}
f_M(v) \d v =  2F_{X}(y) f_{Y}(x)\d x \implies f_M(v) =  2F_{X}(v) f_{Y}(v).
\end{align}
\end{solution}
\end{exercise}
...
\begin{align*}
  f_{L,M}(u,v)\1{u\leq v}= 6 f_{X}(u) (F_{Y}(v)-F_{Y}(u)) f_{Z}(v)\1{u\leq v}.
\end{align*}
