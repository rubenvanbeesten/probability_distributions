...
It is easy to use, and prevents many mistakes.
...
Here is a figure that I use to memorize the logic. The left panel corresponds to 1D transformations, the right to 2D transformations. 
In the explanation below, I only sketch ideas, and skip the technical conditions; you can find those in part in the book, and in detail on wikipedia.
...
For the left panel, we are given a density $f_X$ that maps a point $x$ to a real number, and we are given a function $g$ that maps $x$ to $y$. The problem is to get the density $f_Y$ that can take us from $y$ to $\R$.  For this we can use the concept of /pullback/: start with $y$, compute $x=g^{-1}(y)$ from this, and then set
\begin{align*}
f_Y(y)= f_{X}(g^{-1}(x)).
\end{align*}
In words, we start with $y$, then we apply $g^{-1}$ to pull back $y$ to $x$, and once we are at $x$, we can apply $f_{X}$. 
...
Again, we want to get the density $f_{U,V}$ that can take us from $(u,v)$ to $\R$.  With  a pullback we set
\begin{align*}
f_{U,V}(u,v) := f_{X,Y}(g^{-1}(u,v)),
\end{align*}
where $(x,y)= g^{-1}(u,v)$.

For the left panel again, observe that for the CDFs we can again use pullback to see that $F_Y(y)) = F_X(g^{-1}(y))$.  If you realize that $y=g(x)$, then we can rewrite this to
$F_{Y}(g(x)) = F_X(x)$. Taking the derivative with respect to $x$ at the LHS and RHS we obtain
\begin{align*}
f_Y(g(x)) g'(x) = f_X(x) \iff  f_Y(y) \frac {\d y }{\d x} = f_X(x) \iff f_Y(y) \d y= f_X(x) \d x.
\end{align*}
But this says, conceptually, that the `probability content' of an event should remain the same after a transformation under $g$. The RHS expression is the most useful to memorize; with this, you cannot go wrong in the transformations.

For the 2D case this works exactly the same. Since  $F_{U, V}(g(x,y)) = F_{X,Y}(x, y)$, 
\begin{align*}
f_{U,V}(u,v) | \frac{\partial (u,v)}{\partial (x,y)} | = f_{X,Y}(x,y) \iff 
f_{U,V}(u,v) \d u \d v = f_{X,Y}(x,y) \d x \d y.
\end{align*}
Conceptually, the Jacobian at the left tells us how area changes under the transformation under $g$ (compare scaling from m^2 to km^2).  Again, memorize the RHS.


For the exercises below, I often made this figure not to mess up the sequence of steps.
I advice you to do the same.
And for those of you that do maths and/or theoretical physics as a second study, you'll encounter pullbacks later in your career too, in differential geometry as exterior algebra, and in (general) relativity.

You should know that the transformation here works in the reverse direction of what you see in calculus.
In the calculus setting, we typically have two functions, $g$ and $h$, where $y=g(x)$ and $z=h(y)$ and we want to see how $z$ depends on $x$.
For this, we consider the composition $z = (h\circ g) (x) = h(g(x))$, and then $\d z/ \d x = h'(g(x)) g'(x)$ by the chain rule.
In probability theory, we consider a r.v.
$Y$ as a function $g$ of a r.v.
$X$, i.e., $Y=g(X)$.
Now, we want to find $F_{Y}$, which we compute as follows:
\begin{align*}
F_{Y}(y) = \P{Y\leq y} = \P{g(X) \leq y} = \P{ X \leq g^{-1}(y)} = F_X(g^{-1}(y)),
\end{align*}
assuming that $g$ is increasing.
Hence, $F_Y = F_X\circ g^{-1}$, i.e., with $g^{-1}$ instead of $g$.

So, why is that?
Well, if we want to define $Y$ as a function of $X$, then we see in the above equation that the CDFs must work with $g^{-1}$.
If we would like to use $g$ in the CDFs, then we would get $Y=g^{-1}(X)$.
So, either way, there the inverse of $g$ is involved.
When you learn more about probability theory, you will see that setting $Y=g(X)$ and $F_{Y} = F_X\circ g_{-1}$ is more convenient.
