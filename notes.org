...
** Example 7.1.24
This works best with indicator functions.
In fact, I often use indicators to restrict the area over which to integrate or compute probabilities (recall, $\P{A} = \E{\1{A}}$ for an event $A$).
...
*** 7.1
Let $A$ the arrival time of Alice, and $B$ the time of Bob. Then we want to compute $\P{|A-B|\leq 1/4}$.  (15 minutes is 1/4 hour.)  Why is $f_{A,B}(a,b) = \1{a\in[0,1]}\1{b\in [0,1]}$? Now apply 2D-LOTUS to the function $g(a,b) = \1{|a-b|\leq 1/4}$. 
...
\begin{align*}
f_{Y,Z}(y, z) = \frac 1 {2\pi} \frac 1 {\sqrt{3}} e^{-y^2/2 - z^2/2 + (y+z)^2/18}.
\end{align*}
...
Then $S_n=\sum_{i=1}^n X_i$ and $T_n=\sum_{j=1}^n Y_{j}, and $R_n^2= (S_n+T_n)^2$. For your convenience, I'll stick to the notation of the book.
...
1. /bilinearity/: $\cov{aX, Y + Z}=a\cov{X,Y} + a\cov{X, Z}$ for $a\in \R$. 
...
1. /bilinearity/: $D(a x,y+z) = a D(x,y) + a D(x, z)$.
2. $D(x,x)$ = 0
3. $D( (1,0), (0,1)) = 1$.
...
\end{align} 
However, when $g$ is not increasing everywhere, finding $F_Y$ in terms of $F_X$ is not so simple anymore because there are multiple points $x$ such that $g(x)=y$.
Moreover, when we want to generalize to 2D cases (or yet more dimensions), working with CDFs becomes messy.
Of course a good strategy to prevent dealing with complications is to avoid, or ignore, them.
But this does not work in probability theory.
Here are two examples.

In 1D, take $g(x)= x^{2}$. We know that $\V{X} = \E{X^2}-(\E X)^2$. But $\E{X^2}= \E{g(X)}$, and $g$ is not monotone increasing. Hence, even to define the variance, we already have to deal with a function that is not one-to-one everywhere.
In 2D, functions can have  saddle points, i.e., points in which the function increases in one direction and decreases in another. Then finding the set of points $x$ such that $g(x)\leq y$ is not an attractive task, to say the least.\footnote{We say that $x<y$ on $\R^2$ when both $x_1<y_1$ and $x_2< y_2$.}

The easiest way to characterize $Y=g(X)$ is to work with the densities $f_Y$ and $f_X$.
Then we only have to require /locally/ that $g$ is one-to-one, and we don't have to work with inequalities, but can focus on /points/ at which $g(x) = y$.  Thus, below we will concentrate on the transformation of densities. 

To organize the relations between the r.v.s and $g$,  I always draw the figure below. The left panel corresponds to 1D transformations, the right to 2D transformations. 
\begin{center}
\begin{tabular}{cc}
...
\end{tabular}
\end{center}
For the left panel, we are given a density $f_X$ that maps a point $x$ to a real number $f_X(x)$, and we are given a function $g$ that maps $x$ to $y$, i.e, $y=g(x)$.
The problem is to express  the density $f_Y$, that maps $y$ to $\R$, in terms of $g$ and $f_{X}$. 
For the right panel, we are given a (joint) density $f_{X,Y}$ that maps a point $(x,y)$ to a real number, and we are given  a transformation $g$ that maps $(x,y)$ to $(u,v)$.
Again, we want to express  the density $f_{U,V}$, that maps $(u,v)$ to $\R$, in terms of $g$ and $f_{X,Y}$.

For the 1D case, we can take the derivative at the LHS and RHs of \cref{eq:1D-CDF}. Writing $x=g^{-1}(y)$, this gives with the chain rule
\begin{align*}
f_{Y}(y) = F_{Y}'(y) = \frac{\d }{\d y} F_{X}(g^{-1}(y)) = f_{X}(x) \frac{\d }{\d y} g^{-1}(y).
\end{align*}
To get the derivative of $g^{-1}$, we can apply the chain rule:
\begin{align*}
...
\end{align*}
There is one subtlety, when $g$ is decreasing at $x$, $f_{Y}(y) = -F_Y(y)$, but since at such a point we also have $g'(x)<0$, so that the signs cancel. Hence, in general we have
\begin{align*}
f_{Y}(y) =f_{X}(x)\frac 1{|g'(x)|}.
\end{align*}
...
Well, we can split the line into disjoint intervals in which $g$ is either strictly increasing or decreasing, and then we apply the above rule in each of intervals.
Here, see that $g'(x)= 2x$, hence,
\begin{align*}
...
For the 2D case, we need the following theorem of analysis, which is not entirely straightforward to prove, hence we skip the proof. Let $A$ be the set of points $(x,y)$ such that $g(x,y) \in B$; in other words, $A$ is the inverse of $B$, i.e., $A=\{(x,y): g(x,y) = (u,v)  \in B\}$. Then it can be proven that
\begin{align*}
\iint_{B} f_{U,V}(u,v) \d u \d v = \iint_{A}f_{U,V}(g(x,y)) \frac{\partial (u,v)}{\partial (x,y)} \d x \d y,
\end{align*}
where $\partial(u,v)/\partial(x,y)$ is the absolute value of the Jacobian of $g$ computed at $(x,y)$. 
...
For a bit more detail, see the appendix of BH, for yet more detail check the web, and if you really like such stuff, read the following books of Bressoud (in that order): 
Second Year Calculus: From Celestial Mechanics to Special Relativity, A Radical Approach to Real Analysis, and A Radical Approach to Lebesgue's Theory of Integration. 

Now, to relate $Y$ to $X$, remark that $\P{X\in A} = \P{Y \in B}$, i.e., 
\begin{align*}
...
There simple way to write the above to help memorize things. We have that $g(x,y) = (u,v)$ and we want the probability to be conserved under the transformation $g$. From this,
\begin{align*}
...
Compute $\E X$ for $X\sim \Exp{\lambda}$ by converting integral to the form $\int x e^{-x} \d x$ with a coordinate transform.

Solution:  Here is my standard method. Writing $a$ and $b$ for the boundaries to show explicitly what is going on, 
\begin{align*}
\int_{a}^b x \lambda e^{-\lambda x} \d x = 
\int_{\lambda a}^{\lambda b} u e^{-\lambda x} \frac{\d u}\lambda
\end{align*}
by taking the substitution $u= \lambda x$, so that $\d u = \lambda \d x \implies \d x = \d u /\lambda$. So, I replace  $\lambda x$ by $u$, $\d x$ by $\d u/\lambda$, and I convert the boundaries to $\lambda a$ and $\lambda b$. 

How does this work when we take $g(x)= \lambda x $, and take $Y=g(X)$? There are two ways to compute $\E Y$. The first is by 1D LOTUS:
\begin{align*}
\E Y = \E{g(X)} = \int g(x) f_X(x) \d x= \int \lambda x \lambda e^{-\lambda x} \d x = 1;
\end{align*}
as there is a $\lambda x$ everywhere, I can just substitute $\lambda x$ for $u$. Note the, perhaps, surprising result that when we take $g(x) = \lambda x$, we convert $X\sim \Exp{\lambda}$ into  $Y\sim \Exp{1}$!
Now with a change of variable $g(x) = \lambda x$: 
\begin{align*}
\E Y &= \int y f_Y(y) \d y, \\
f_Y(y) &= f_X(g^{-1}(y)) \frac {\d x}{\d y } = \lambda e^{-\lambda g^{-1}(y)} (\d y/ \d x)^{-1} = \lambda e^{-\lambda y/ \lambda} \lambda^{-1} = e^{-y} \\
\E Y &= \int y e^{-y} \d y = 1.
\end{align*}
The result is the same (as it should).

So, what do we do we take $X \sim \Exp{\lambda}$ so that $f_X(x)= \lambda e^{-\lambda x}$?
Then we change the density in the `calculus direction', not in the `probability direction'! In other words, we follow the sequence $f_Z \circ g$ for $Z\sim \Exp{1}$, in other words, we set $z$ to $\lambda x$ and then we substitute $\lambda x$ in the density $e^{-z} \d z = e^{-\lambda x} \d (\lambda x) = \lambda e^{-\lambda x} \d x$.  And, as we saw already above, this is not the same as taking $X=g(Z)$, it's the `reverse'. 

Be aware, it's easy to mess up; I do it regularly, but I know that I have to pay attention, and I check! And that helps (most of the time).



** TODO Jaynes, sterren, normale verdeling
** Example 8.1.6
...
** TODO Definition 9.2.1
This definition is subtle, and it takes time to understand.
Here is a slightly different explanation (For me, different explanations often helps to understand; if one does not stick, often another one does.)
Take some random variable $X$, say.
Then, as in Chapter 7, or in Example 9.1.7, we are interested in the expectation of $g(X)$.
Specifically, for Example 9.1.7, I computed above the expectation of $g(V)$ with $g(v)= (v-b)\1{\alpha v\leq b}$.

For conditional expectation, we can compute $\E{Y \given X=x}$, that is, we compute $\int f_{Y| X}(y | x) \d y$ when $Y$ is continuous.
Observe now that this is just a function of $x$: $g(x) = \int f_{Y| X}(y | x)\d y$. And, as earlier, we can be interested in computing the expectation of $g(X)$. It is true that $X$ plays some sort of double role---first we use it in the conditioning to find $g$, and then we plug it into  $g$ again---and this is perhaps confusing. But I finally `got  it', when I understood that $g$ can be interpreted as just some function, and then we compute $\E{g(X)}$.

The letter $E$ is a bit overloaded at times. Sometimes we take the expectation with respect to the joint PMF, sometimes with respect to $f_X$, sometimes with respect to $f_{Y|X}$. It can be helpful to label the expectation to make explicit what we mean. For instance, in
\begin{align*}
\E{Y\given X} = \int y f_{Y|X}(y|x) \d y
\end{align*}
we integrate over $y$, /not/ over  $x$ and $y$, while in
\begin{align*}
\E{Y} = \iint y f_{X,Y}(x, y) \d x \d y = \int y f_Y(y) \d y,
\end{align*}
we do integrate over $x$ and $y$, and use marginalization to obtain the second equation. Hence, it might be helpful to write $\E{Y\given X}{Y}$ in the former to show that intgegrate over $y$. And then it is clear that
\begin{align*}
\E Y = \E{\E{Y|given X}{Y}}{X},
\end{align*}
because in the RHS the last expectation is over $x$ (with density $f_X$). 
...
Here are the solution; for the moment, I don't know a set of good hints to coach to the answer. If you do, tell me.

**** a. 

# Here you should assume that the $X_i$ are not yet known. Thus, the expectation over $X_i$ is taken with respect to the CDF $F_X$. 

Suppose $S_{j}\sim \DUnif{\{1, \ldots, n\}}$ is the \(j\)th sample. Then, $X_j^{*} = \sum_{i=1}^n X_i\1{S_j = i}$. And then, using the independence of $X_j$ and $S_j$, 
\begin{align*}
\E{X_j^*}&= \sum_i \E{X_i}\E{\1{S_{j}=i}} = \mu, \\
\V{X_j^*}&= \E{(\sum_{i} X_{i}\1{S_j=i})^{2}} - (\E{X_{j}})^{2}=  \E{X^2} - \mu^2=\sigma^{2}.
\end{align*}
...
\begin{align*}
\E{\hat X^{*}\given X_1,\ldots,X_n} &=  \frac 1 n \E{\sum_{j} X^{*}_j\given X_1,\ldots,X_n} 
=\frac 1 n \E{\sum_{j} \sum_{i} X_{i} \1{S_j=i}\given X_1,\ldots,X_n}  \\
& =\frac 1 n \sum_{i} X_{i} \sum_{j}\E{\1{S_j=i}}  
 =\frac 1 n \sum_{i} X_{i}.
\end{align*}
This is not the same as $\mu$!.
The conditional variance:
\begin{align*}
\V{\bar X^{*} | X_1, \ldots X_n} =
%\V{\frac 1 n \sum_{j} \sum
%\frac 1 {n^{2}} \V{\sum_{j} X_{j}^{*} | X_1, \ldots X_n} = \frac 1 n \V{X^{*}} = \frac 1 n \E{X^{*}^2} - \bar }
\end{align*}

**** c.  
$\E{\bar X^{*}}$ is simple, use linearity and the tower property. For the variance, use Eve's law. Here are the details for the latter.
\begin{align*}
\E{\V{\bar X^{*} | X_1, \ldots X_n}} &=  \E{\frac 1 n \sum_{i=1}^n (X_i-\bar X)^2} = \E{(X-\bar X)^2}, \\
\E{(X-\bar X)^2} &=  \E{X^{2}} -\E{\bar X^2} \\
\E{\bar X^2} \\
\end{align*}
...
**** a. 
I use recursions, because I like that:
\begin{align*}
...
* TODO derive density for sum,product, and ratio of two normal r.v. by means of infinitesimals.
