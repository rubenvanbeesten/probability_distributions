...
In 2D, a function can have saddle points, i.e., points in which the function increases in one direction and decreases in another.
Then finding the set of points $x$ such that $g(x)\leq y$ (which we need for $\P{g(X) \leq y}$)is not an attractive task, to say the least.\footnote{We say that $x<y$ on $\R^2$ when both $x_1<y_1$ and $x_2< y_2$.}
Finally, consider the maximum of a number of r.v.s, which is interesting for people that like to bet on horse races or 100 m sprint events.
Then, as long $X<Y$, the second component of $g(X,Y)=(\min{X,Y}, \max{X,Y})$ remains constant under changes in $X$. 
...
\end{align*}
As this holds for any (reasonable) set $B$, 
\begin{align*}
...
\end{align*}
As a piece of advice, memorize the LHS of these two formulas and the figures above. With these you can derive the rest. 


The change-of-variables formula for integration relates directly to the substitution rule to solve 1D integrals. Specifically, when we have two 1D functions $f$ and $g$, then the substitution rule says that,
\begin{align*}
